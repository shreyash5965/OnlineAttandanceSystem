...
if 'user_data' in st.session_state:
    ...
    tab1, tab3 = st.tabs(['Registered Data', 'Attendance Report'])
    with tab1:
        if st.button('Refresh Data'):
            with st.spinner('Retriving data from Redis database'):
                registration_data = realTimePrediction_pg.retrieve_data()
                st.dataframe(registration_data[['Name', 'Email', 'Role', 'Facial Features']])
    # with tab2:
    ...
    with tab3:
        ...
        logs_df = realTimePrediction_pg.get_log_data(user_data["id"], '', '')
        ...
        report_df = logs_df.groupby(by=['Date', 'Name', 'Role', 'Subject']).agg(
        ...
        name_role = report_df[['Name', 'Role', 'Subject']].drop_duplicates().values.tolist()

        date_name_role_zip = []
        for date in unique_dates:
            for name, role, subject in name_role:
                date_name_role_zip.append([date, name, role, subject])

        date_name_role_zip_df = pd.DataFrame(date_name_role_zip, columns=['Date', 'Name', 'Role', "Subject"])
        # left join the dataframe with report df to find all student/teacher not present in the given day

        date_name_role_zip_df = pd.merge(date_name_role_zip_df, report_df, how='left', on=['Date', 'Name', 'Role', 'Subject'])
